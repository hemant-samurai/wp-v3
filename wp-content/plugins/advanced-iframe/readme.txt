...
[advanced_iframe securitykey=""   src="" 
  ...
  include_html = ""   enable_ios_mobile_scolling = ""
...
- Fix: show_iframe_as_layer_header_position === 'bottom' was fixed.  
...
- Fix: show_iframe_loader, hide_part_of_iframe and hide_page_until_loaded are now working properly. hide_part_of_iframe elements are now hidden until the iframe is loaded.    
...
- Fix: Zoom and php < 5.3 does not define a needed variable. Now this is defined for older php versions.

= 7.2 =
- New: {timestamp} can be used in the src. It adds a timestamp which can be used to avoid caching of iframes.
- New: element_to_measure is now available on the external workaround tab directly-> Additional height
- New: With post message you can now mix https and http in the external workaround.
- Fix: Resize later on the external workaround is now working again.
