...
$current_user = wp_get_current_user();
 ...
 $usermeta_html .= '</span>';
        ...
        $src_text =  __('Enter the full URL to your page. e.g. http://www.tinywebgallery.com. <strong>Please do not use a different protocol for the iframe: Do not mix http and https if possible! Http pages are NOT shown in https pages.</strong> Please read <a href="http://www.tinywebgallery.com/blog/iframe-do-not-mix-http-and-https" target="_blank">this post</a> for details. If you cannot save the full url because of mod_security don\'t specify the protocoll (e.g. //www.tinywebgallery.com) or leave this field empty and define the src in the shortcode. Also use the free url checker below to make sure that you can include the page. You can also add parameters to this url like http://www.tinywebgallery.com/test.php?iframe=true. Then you can check this variable and use it to e.g. hide some elements in the iframe.<br />The pro version also has some placeholders (the standalone version has only host and port available) which are replaced on the fly: <ul class="ai-src-list"><li><strong>{site}</strong>: the url to the wordpress root</li><li><strong>{host}</strong>: the current host from the request</li><li><strong>{port}</strong>: the current port from the request</li><li><strong>{userid [,defaultvalue]}</strong>: the id of the current user. The optional defaultvalue is used if no user is logged in.</li><li><strong>{username [,defaultvalue]}</strong>: the username of the current user. The optional defaultvalue is used if no user is logged in.</li><li><strong>{useremail [,defaultvalue]}</strong>: the e-mail of the current user. The optional defaultvalue is used if no user is logged in.</li><li><strong>{adminmail}</strong>: the e-mail of the wordpress admin</li><li><strong>{userinfo-X [,defaultvalue]}</strong>: extract attribute X from get_currentuserinfo. E.g. {userinfo-display_name}. The optional defaultvalue is used if the attribute is not found or set. See <a href="https://codex.wordpress.org/Function_Reference/wp_get_current_user" target="_blank">here</a> for details. '. $userinfo_html.'</li><li><strong>{usermeta-X [,defaultvalue]}</strong>: extract key X from get_user_meta. E.g. {usermeta-last_name}. The optional defaultvalue is used if the attribute is not found or set. See <a href=" https://codex.wordpress.org/Function_Reference/get_user_meta" target="_blank">here</a> for details. '. $usermeta_html.'</li><li><strong>{href}</strong>: The full url that is shown in the address bar</li><li><strong>{urlpathX}</strong>: the Xth path element from the front. The first path element would be {urlpath1}<</li><li><strong>{urlpath-X}</strong>: the Xth path element from behind. The last path element would be {urlpath-1}</li><li><strong>{query-X [,defaultvalue]}</strong>: the value of the query parameter sent by GET or POST. ?example=myvalue would be {query-example} -> myvalue. The optional defaultvalue is used if the parameter is not found.</li><li><strong>{timestamp}</strong>: a timestamp which can be used to avoid caching of iframes</li></ul>Make sure that <strong>no spaces are in the placeholders</strong>.<br/>All placeholders except {site}, {host}, {port} are urlencoded! An example would be src="http://demo.{host}/url?id={userid}". Especially for multidomain installations this is maybe helpful. If no user is logged in the values are empty or 0 for the id.<br />urlpath does extract path elements from the url in the address bar. So {urlpath-1} for the url www.xx.com/a/bb/cc would be cc. All placeholders that cannot be resolved are removed.<br /><strong>Also shortcodes are supported.</strong> You have to replace the bracket [ with {{ and ] with }}. So if the shortcode is [link] you have to use {{link}} because shortcode attributes which include shortcodes are not supported directly by Wordpress. Also be aware of single and double quotations: src="http://demo.{{url domain=\'home\'}}/url". So only use \' for attributes of the nested shortcode.<br /><strong>PDF support: </strong>If you include a pdf google doc is used to render the pdf. This solution looks the same on all browsers. If you want to use the native pdf renderer of the browser/your system add NATIVE: before the url. Like NATIVE:http://www.example.com/pdf.pdf.<br />Shortcode attribute: src=""', 'advanced-iframe');
} else {
